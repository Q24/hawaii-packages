...
export class TokenService {
  ...
  private static getStoredTokens(): Token[] {
    const storedTokens = StorageUtil.read(
      `${configService.config.client_id}-token`
    );
   ...
   */
  private static storeTokens(tokens: Token[]): void {
   ...
   */
  private static cleanExpiredTokens(storedTokens: Token[]): Token[] {
   ...
   */
  private static saveIdTokenHint(idTokenHint: string): void {
    StorageUtil.store(
      `${configService.config.client_id}-id-token-hint`,
      idTokenHint
    );
  }
