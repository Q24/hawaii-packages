...
export class SessionUtil {
   ...
   */
  private static validateToken(hashParams: Token): Promise<ValidSession> {
    const data = {
   ...
   */
  static checkIfTokenExpiresAndRefreshWhenNeeded(almostExpiredThreshold = 300, token: Token): Promise<boolean> {
    return new Promise((resolve) => {
      if (token.expires && token.expires - Math.round(new Date().getTime() / 1000.0) < almostExpiredThreshold) {
